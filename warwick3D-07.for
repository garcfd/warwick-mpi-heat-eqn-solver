...
C     run:     mpirun -np 16 ./warwick
      ...
      REAL, DIMENSION(1:nx, 1:ny, 1:nz) :: values
      ...
      INTEGER :: ierr
      REAL, DIMENSION(1:nx, 1:ny, 1:nz) :: red
     ...
     &       values_local(1:nx_local, 1:ny_local, 1:nz_local)
      ...
      CALL MPI_Reduce(values, red, nx*ny*nz,
...
c      write(6,*) "xmin/xmax ",x_min_rank," ",x_max_rank
c      write(6,*) "ymin/ymax ",y_min_rank," ",y_max_rank
c      write(6,*) "zmin/zmax ",z_min_rank," ",z_max_rank

      values = 0.0
      values_local = 0.0

      if (x_min_rank.eq.-2)  values_local(0,:,:) = 1.0 ! xmin
      if (y_min_rank.eq.-2)  values_local(:,0,:) = 1.0 ! ymin
      if (z_min_rank.eq.-2)  values_local(:,:,0) = 1.0 ! zmin

      if (x_max_rank.eq.-2)  values_local(nx_local+1,:,:) = 1.0 ! xmax
      if (y_max_rank.eq.-2)  values_local(:,ny_local+1,:) = 1.0 ! ymax
      if (z_max_rank.eq.-2)  values_local(:,:,nz_local+1) = 1.0 ! zmax
